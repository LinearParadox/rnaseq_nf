...
args=c("gene_out_paths.csv", "human", "design_matrix.csv", "contrast_matrix.csv")
args = commandArgs(trailingOnly = T)
samples = read.csv(args[[1]], header=F)
colnames(samples) <- c("names", "files")
rownames(samples) <- samples[,1]
design <- as.matrix(read.csv(args[[3]], header=T, row.names=1))
coef <- as.matrix(read.csv(args[[4]], header=T, row.names=1))
...
catch <- catchSalmon(samples[,2], )
divided.counts <- catch$counts/catch$annotation$Overdispersion
colnames(divided.counts)<-rownames(samples)[match(colnames(divided.counts), samples$files)]
y <- DGEList(counts = divided.counts,
              samples = samples,
             genes = catch$annotation)
rownames(y$samples) <- y$samples$names
